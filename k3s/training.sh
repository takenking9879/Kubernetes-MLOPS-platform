...
# TIMEOUTS
# ============================================================
SPARK_DRIVER_TIMEOUT="130"
RAY_HEAD_TIMEOUT="120"
SPARK_COMPLETION_TIMEOUT="1800"
RAY_COMPLETION_TIMEOUT="3600"
...
deploy_spark_preprocessing() {
  sep
  log_info "Deploying Spark Preprocessing"

  log_info "Applying Spark preprocessing driver service"
...
deploy_ray_training() {
  ...
  if [ $ELAPSED -ge $RAY_HEAD_TIMEOUT ]; then
    log_warn "Timeout waiting for Ray head. Continuing anyway..."
  fi

  ok "Ray training deployed"
}
...
cleanup_resources() {
  ...
  if [ "$ENABLE_TRAINING" = "true" ]; then
    ...
    ELAPSED=0
    RAY_STATE=""
    while [ $ELAPSED -lt $RAY_COMPLETION_TIMEOUT ]; do
...
if [ "$ENABLE_PREPROCESS" = "true" ]; then
  ...
  echo "    - Prometheus:       spark-preprocess-driver-svc.spark:8001/metrics"
  echo "    - Spark Metrics:    spark-preprocess-driver-svc.spark:4040/metrics/prometheus"
fi
...
if [ "$ENABLE_TRAINING" = "true" ]; then
  ...
  echo "    - Prometheus:       kuberay-training-metrics-svc.ray:8002/metrics"
fi
...
sep
ok "TRAINING PIPELINE DEPLOYMENT FINISHED SUCCESSFULLY"
