...
class IcebergDatasource(Datasource):
    ...
    def __getstate__(self):
        """Prepare datasource for pickle serialization.

        The live PyIceberg Table object (``self._table``) and its cached
        ``plan_files`` carry non-serializable ``_thread.lock`` objects from
        the S3FileIO thread pool.  Workers will lazily recreate them from
        ``_catalog_kwargs`` (a plain dict — fully serializable).
        """
